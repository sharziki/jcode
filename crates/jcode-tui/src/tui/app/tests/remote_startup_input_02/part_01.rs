...
#[test]
fn test_alt_q_toggles_queue_mode() {
    let mut app = create_test_app();
...
#[test]
fn test_ctrl_t_toggles_full_transcript() {
    let mut app = create_test_app();
    app.full_transcript_visible = false;

    app.handle_key(KeyCode::Char('t'), KeyModifiers::CONTROL)
        .unwrap();
    assert!(app.full_transcript_visible);
    assert_eq!(
        app.status_notice(),
        Some("Transcript: FULL · Ctrl+T returns to focused output".to_string())
    );
